...
> 本包实操提示：如果任务要求创建后正文必须可见并发布，优先使用 `create-doc --create-mode empty` 创建空文档，再用 `edit-content --editor-mode append` 写正文，随后 `publish-doc` 并 `query-content` 读回验证。`--content` 和 `--md-file` 是官方支持的创建模式，但自动化场景仍要读回确认正文是否进入可见正文区。
