...
此 skill 面向 `ruliu-docs` 文件夹，提供如流/KU 文档读取、搜索、创建、编辑、发布、删除、子文档、表格、图片、附件和图示内容写入能力。技能名和文件夹名固定为 `ruliu-docs`。

## 目录结构

- `bin/`：KU 官方 CLI 和配置，优先直接调用 `bin/ku-darwin-arm64`。
- `references/`：KU 官方命令说明。查完整参数、响应字段、权限、评论、附件、在线表格、数据表时读这里。
- `scripts/`：UGate 缓存、依赖检查、企业内搜脚本。
- `experiment/`：实战经验沉淀，包括 JSON 编辑、append/cover、表格、图片、写作风格和复杂节点模板。

## 先读哪些文件

- 普通读、创建、追加、删除、搜索：直接按本文命令做。
- 查 KU CLI 完整命令、参数、响应字段、在线表格、数据表、权限、评论、附件、版本、复制、移动、删除：读 `references/API_INDEX.md`，再读对应子命令文档，例如 `query_content.md`、`create_doc.md`、`edit_content.md`。
- 使用 `edit-content --editor-mode mdsl` 做局部编辑前：读 `references/edit_content.md` 和 `references/mdsl_edit_agent.md`。
- 替换、删除中间内容、插入到指定标题下、改表格某一行：先读 `experiment/ku-doc-editing.md`。
- 写产品方案、PRD、决策文档、需求分析：先读 `experiment/ku-doc-writing/README.md`，必要时读其中 examples。
- 表格、图片、附件、HTML demo 截图、Mermaid/流程图：先读 `experiment/ruliu-writing-tips.md`，复杂节点再读 `experiment/ku-doc-editing.md`。

## 最小运行文件

- KU CLI：`bin/ku-darwin-arm64`
- KU 配置：`bin/config.yaml`
- 企业内搜：`scripts/enterprise-search/`
...
```bash
bash -lc 'export SANDBOX_USERNAME="<uuap>"; SKILL="$HOME/.codex/skills/ruliu-docs"; KU="$SKILL/bin/ku-darwin-arm64"; "$KU" query-user-info --username "$SANDBOX_USERNAME"'
```
...
```bash
bash -lc 'export SANDBOX_USERNAME="<uuap>"; SKILL="$HOME/.codex/skills/ruliu-docs"; KU="$SKILL/bin/ku-darwin-arm64"; "$KU" query-content --url "https://ku.baidu-int.com/knowledge/..." --protocol markdown --show-doc-info'
```

读编辑器 JSON：

```bash
bash -lc 'export SANDBOX_USERNAME="<uuap>"; SKILL="$HOME/.codex/skills/ruliu-docs"; KU="$SKILL/bin/ku-darwin-arm64"; "$KU" query-content --url "https://ku.baidu-int.com/knowledge/..." --protocol json --show-doc-info'
```
...
```bash
bash -lc 'export SANDBOX_USERNAME="<uuap>"; SKILL="$HOME/.codex/skills/ruliu-docs"; KU="$SKILL/bin/ku-darwin-arm64"; INFO="$("$KU" query-user-info --username "$SANDBOX_USERNAME")"; REPO_ID="$(printf "%s" "$INFO" | sed -n "s/.*\"repositoryGuid\": *\"\([^\"]*\)\".*/\1/p" | head -1)"; if [ -z "$REPO_ID" ]; then echo "ERROR: repositoryGuid not found" >&2; exit 1; fi; CREATE="$("$KU" create-doc --repo-id "$REPO_ID" --username "$SANDBOX_USERNAME" --title "文档标题" --create-mode empty --process-images=false)"; DOC_ID="$(printf "%s" "$CREATE" | sed -n "s/.*\"docGuid\": *\"\([^\"]*\)\".*/\1/p" | head -1)"; if [ -z "$DOC_ID" ]; then echo "ERROR: docGuid not found" >&2; printf "%s\n" "$CREATE"; exit 1; fi; OPS='\''[{"mode":"append","withNewCard":true,"json":[{"type":"paragraph","children":[{"text":"正文内容"}]}]}]'\''; "$KU" edit-content --doc-id "$DOC_ID" --username "$SANDBOX_USERNAME" --editor-mode append --operations "$OPS"; "$KU" publish-doc --doc-id "$DOC_ID" --username "$SANDBOX_USERNAME"; "$KU" query-content --doc-id "$DOC_ID" --protocol markdown --show-doc-info'
```

创建子文档：KU 文档可当目录用。传父文档 ID：

```bash
bash -lc 'export SANDBOX_USERNAME="<uuap>"; SKILL="$HOME/.codex/skills/ruliu-docs"; KU="$SKILL/bin/ku-darwin-arm64"; CREATE="$("$KU" create-doc --repo-id "<repositoryGuid>" --parent-doc-id "<parentDocGuid>" --username "$SANDBOX_USERNAME" --title "子文档标题" --create-mode empty --process-images=false)"; DOC_ID="$(printf "%s" "$CREATE" | sed -n "s/.*\"docGuid\": *\"\([^\"]*\)\".*/\1/p" | head -1)"; if [ -z "$DOC_ID" ]; then echo "ERROR: docGuid not found" >&2; printf "%s\n" "$CREATE"; exit 1; fi; OPS='\''[{"mode":"append","withNewCard":true,"json":[{"type":"paragraph","children":[{"text":"正文内容"}]}]}]'\''; "$KU" edit-content --doc-id "$DOC_ID" --username "$SANDBOX_USERNAME" --editor-mode append --operations "$OPS"; "$KU" publish-doc --doc-id "$DOC_ID" --username "$SANDBOX_USERNAME"; "$KU" query-content --doc-id "$DOC_ID" --protocol markdown --show-doc-info'
```
...
```bash
bash -lc 'export SANDBOX_USERNAME="<uuap>"; SKILL="$HOME/.codex/skills/ruliu-docs"; cd "$SKILL/scripts/enterprise-search"; python3 ku_search.py --word "关键词" --page 1 --page-size 10'
```

企业内搜：

```bash
bash -lc 'export SANDBOX_USERNAME="<uuap>"; SKILL="$HOME/.codex/skills/ruliu-docs"; cd "$SKILL/scripts/enterprise-search"; python3 neisou_search.py --word "关键词" --page 1'
```

会议、周报、OKR、通讯录也在 `scripts/enterprise-search/`，按脚本名选择。通讯录：

```bash
bash -lc 'export SANDBOX_USERNAME="<uuap>"; SKILL="$HOME/.codex/skills/ruliu-docs"; cd "$SKILL/scripts/enterprise-search"; python3 address_search.py --type corpuser --q "姓名或邮箱前缀"'
```
...
- 复杂表格、图片、附件、流程图节点模板见 `experiment/ku-doc-editing.md`。
...
- 先定义口径，再放推算。
- 写完读回检查；如果用户纠正了结构、表格、语气或范围，把可复用经验更新到 `experiment/ku-doc-writing/README.md` 或 examples。
